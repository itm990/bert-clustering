...
def main():
    ...
    parser.add_argument('--train_sent', type=str, nargs='+', default=[None])
    parser.add_argument('--train_cls',  type=str, nargs='+', default=[None])
    parser.add_argument('--valid_sent', type=str, nargs='*', default=[None])
    parser.add_argument('--valid_cls',  type=str, nargs='*', default=[None])
    ...
    train_sent, train_cls = load_sentence_and_label(opt.train_sent, opt.train_cls)
    valid_sent, valid_cls = load_sentence_and_label(opt.valid_sent, opt.valid_cls)
    ...
    training_args = TrainingArguments(
        ...
        do_train=True,
        do_eval=True,
        num_train_epochs=opt.max_epoch,
        logging_dir='{}/logs'.format(opt.output_dir),
        logging_steps=opt.logging_steps,
        evaluation_strategy='steps',
        eval_steps=opt.eval_steps,
        save_steps=opt.eval_steps,
    )
    
    config = BertConfig.from_json_file('{}/config.json'.format(opt.model_name_or_path))
    config.num_labels = opt.class_num
